...
- cat_fastq — only active for samples with more than one fastq pair; not expressible in the port's one-pair-per-sample sample model
- auto strandedness (per-sample inference from the samplesheet) — pipeline-level config.strandedness with forward/reverse/unstranded values only; the inferred per-sample value has no consumer in a static DAG
- PREPARE_GENOME GTF preprocessing chain — CUSTOM_GTFFILTER (GTF filtered to the genome's sequence names), gffread GFF->GTF, additional_fasta (CUSTOM_CATADDITIONALFASTA), GENCODE preprocessing and .gz/.tar.gz reference handling; the gene_bed / chrom_sizes / transcript_fasta derivations ARE ported as prepare_genome::* builder rules (empty config key = derived from fasta + gtf like upstream)
...
| 2 | `PREPARE_GENOME` prepares the reference artifacts (fasta, gtf, transcript_fasta, gene_bed, chrom_sizes) and builds the branch indexes (STAR / HISAT2 / RSEM / Salmon) | The artifact derivations are ported as `prepare_genome::gene_bed` (ea-utils gtf2bed), `chrom_sizes` (samtools faidx) and `transcript_fasta` (RSEM) builder rules — empty config key = derived from fasta + gtf like upstream; the index BUILDERS are ported too: STAR via the `[[references]]` builder, HISAT2 / RSEM / Salmon / Bowtie2 / Kallisto via when-gated builder rules that build from the shipped fixtures when the config key is empty and symlink a user-supplied directory otherwise | Only the GTF preprocessing chain (CUSTOM_GTFFILTER, gffread, additional_fasta, GENCODE, .gz/.tar.gz handling) stays excluded |
| 3 | Non-default branches: `star_rsem`, `hisat2`, `bowtie2_salmon`, `--with_umi`, `--pseudo_aligner salmon`, `--pseudo_aligner kallisto` | Ported — see rows 16-27 for their deviations | `as_quantification` was checked against the 3.26.0 source and does not exist upstream — RSEM always runs `--alignments`, so there is nothing to exclude |
...
| 19 | `RSEM_PREPAREREFERENCE` emits `transcripts.fa` next to the index | The `rsem_index` builder does not emit it — `prepare_genome::transcript_fasta` runs the same derivation (rsem-prepare-reference, `--num-threads` from the rule) when any consumer branch is active (star_salmon / bowtie2_salmon / salmon / kallisto pseudo) | Nothing in the RSEM chain consumes `transcripts.fa`; the align-mode RSEM input is the toTranscriptome BAM |
...
`PREPARE_GENOME`'s GTF preprocessing chain (`CUSTOM_GTFFILTER` — GTF filtered to the genome's sequence names, `gffread` GFF->GTF, `additional_fasta` (`CUSTOM_CATADDITIONALFASTA`), GENCODE preprocessing and `.gz`/`.tar.gz` reference handling); per-sample `min_trimmed_reads` filtering (data-dependent per-sample state; only the MultiQC fail_trimmed table is produced); `cat_fastq` (only active for samples with more than one fastq pair — not expressible in the port's one-pair-per-sample sample model); `auto` strandedness (per-sample inference from the samplesheet — the inferred per-sample value has no consumer in a static DAG); the Nextflow-param-rendered MultiQC sections (`workflow_summary_mqc.yaml` / `methods_description_mqc.yaml`). The gene_bed / chrom_sizes / transcript_fasta derivations are not excluded — they are ported as `prepare_genome::*` builder rules (see row 2).
