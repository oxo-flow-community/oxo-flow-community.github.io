...
<div class="ox-kv"><span class="k">Compute</span><span class="v"><span title="up to 12 CPUs / 72 GB per rule (Cell Ranger)">up to 12 CPUs / 72 GB per rule (Cell Ranger)</span></span></div>
...
```bash
oxo-flow run gh:oxo-flow-community/oxo-flow-scrnaseq
```

Runs straight from the catalog — `oxo-flow` checks the repo out under `.oxo-flow/repos/oxo-flow-scrnaseq` and keeps outputs/checkpoints in the current directory, no manual clone. Pin a revision with `gh:oxo-flow-community/oxo-flow-scrnaseq@<branch-or-tag>`.

Preview the plan first: `oxo-flow pull gh:oxo-flow-community/oxo-flow-scrnaseq` fetches the repo, then `oxo-flow dry-run main.oxoflow`.
